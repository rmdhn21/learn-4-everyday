...
<style>
    ...
    /* Styling Menu Lipat (Expander) agar terlihat 'Clickable' */
...
# ==========================================
# ⚙️ FUNGSI KRUSIAL (JANTUNG APLIKASI)
# ==========================================

# 1. RENDER MATERI INTERAKTIF (INI YANG BOS MINTA PERTAHANKAN)
def render_interactive_content(text):
    ...
    # Bagian pembuka (Intro) sebelum judul pertama
    if sections[0].strip():
        st.markdown(sections[0])
    
    # Loop untuk membuat kotak-kotak materi
    for i in range(1, len(sections), 2):
        if i + 1 < len(sections):
            title = sections[i].replace("##", "").strip()
            content = sections[i+1].strip()
            
            # Buat Expander dengan ikon Buku
            with st.expander(f"📘 {title}", expanded=False):
                st.markdown(content)

# 2. PENCARI JSON (ANTI ERROR KUIS)
def temukan_json_murni(text):
    try:
        # Cari pola [...] di antara teks sampah apapun
...
# 3. AUDIO PLAYER (FIX UNTUK HP)
def generate_audio_memory(text):
    try:
        mp3_fp = BytesIO()
        tts = gTTS(text=text, lang='id')
        tts.write_to_fp(mp3_fp)
        mp3_fp.seek(0) # INI KUNCINYA AGAR BISA DIPUTAR DI HP
        return mp3_fp
    except:
        return None

# 4. DIAGRAM GRAPHVIZ ZOOMABLE
...
def ask_the_brain(provider, model_name, api_key, prompt):
    try:
        if provider == "Google Gemini":
            ...
            time.sleep(1)
            # Temperature 0.1 agar output stabil & konsisten
            ...
            chat_completion = client.chat.completions.create(
                messages=[{"role": "system", "content": "Kamu adalah Dosen Senior yang Terstruktur."}, {"role": "user", "content": prompt}],
                model=model_name, 
                temperature=0.1 # Stabil
            )
...
if not st.session_state.kurikulum:
    st.title("🎓 Guru Saku Ultimate (v46)")
    st.info("Status: Materi Interaktif Aktif, Audio Mobile Fix, Kuis Aman.")

tab_belajar, tab_video, tab_kuis, tab_chat = st.tabs(["📚 Materi (Deep)", "🎬 Audio Guru", "📝 Kuis (15 Soal)", "💬 Tanya Guru"])

# === TAB 1: MATERI (INTERAKTIF TERJAMIN) ===
with tab_belajar:
    if st.session_state.kurikulum and pilihan_bab:
        ...
        if st.button("✨ Buka Materi Lengkap"):
            if not api_key: st.error("API Key kosong.")
                ...
                with st.spinner("Menulis materi Panjang & Menggambar Diagram..."):
                    # PROMPT WAJIB INTERAKTIF (##)
                    p = f"""
                    Saya belajar '{st.session_state.topik_saat_ini}', Bab '{pilihan_bab}'.
                    Gaya: {gaya_belajar}.
                    
                    INSTRUKSI 1 (MATERI): 
                    - Jelaskan secara MENDALAM dan PANJANG (Min 800 kata).
                    - **WAJIB:** Gunakan Heading 2 Markdown (##) untuk setiap Sub-Bab agar menjadi menu interaktif.
                    - Struktur: Pendahuluan -> ## Konsep -> ## Mekanisme -> ## Contoh -> ## Kesimpulan.
                    
                    INSTRUKSI 2 (DIAGRAM): 
                    - Buat Graphviz DOT `digraph G {{...}}` di akhir. 
        ...
        # RENDER MATERI JADI KOTAK-KOTAK (INTERAKTIF)
...
with tab_video:
    ...
    if st.session_state.materi_sekarang:
        ...
        if st.session_state.audio_data:
            ...
            with c1: 
                st.info("🔊 Putar Audio")
                st.audio(st.session_state.audio_data, format="audio/mpeg") # Format MPEG aman buat HP
    ...
    if st.button("🎲 Buat Kuis"):
        if not api_key: st.error("API Key?")
            ...
            with st.spinner("Membuat 15 Soal..."):
                ...
                if data_kuis: st.session_state.quiz_data = data_kuis
                else: st.error("Gagal format JSON. Coba klik lagi."); st.write(res)
